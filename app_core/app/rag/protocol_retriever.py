...
def retrieve_protocols(
    *,
    ...
    # IMPORTANT: if nothing matched (score <= 0), do NOT arbitrarily pick the first protocol.
    # That causes incorrect routing (e.g., back pain -> abdominal pain) and cascades into
    # wrong follow-up questions and premature disposition. Fall back to "no protocol".
    if primary_score <= 0:
        primary_doc = None
    ...
    if not primary_doc:
        return {
            ...
            "red_flag_union": list(case_result.get("risk_flags", []) or []),
            "evidence_refs": list(case_result.get("case_refs", []) or []),
            "retrieval_score": 0.0,
            "fallback_used": True,
            "protocol_doc": {},
