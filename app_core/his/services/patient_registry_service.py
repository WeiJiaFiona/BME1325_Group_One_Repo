...
def register_patient(patient: PatientRecord, storage: HisStorage | None = None) -> PatientRecord:
    return resolve_storage(storage).upsert_patient(patient)


def get_patient(patient_id: str, storage: HisStorage | None = None) -> PatientRecord | None:
