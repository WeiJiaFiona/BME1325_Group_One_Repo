...
def triage_cn_ad(case: TriageInput) -> TriageDecision:
    ...
    pain_score = case.vitals.get("pain_score", None)
    try:
        pain_score_f = float(pain_score) if pain_score is not None else None
    except Exception:
        pain_score_f = None
    ...
    if spo2 < 85 or sbp < 80:
        ...
        acuity = "B"
    # High pain score alone is not always resus, but should not default to CTAS 3.
    # Treat severe pain (>=8/10) as at least level B in this teaching simulator.
    elif pain_score_f is not None and pain_score_f >= 8:
        acuity = "B"
