    ...
    def move(self, maze, personas, curr_tile, curr_time, data_collection):
        ...
        if(self.scratch.time_to_next):
            ...
            if queue and curr_time:
                ...
                if not last_aged or (curr_time - last_aged) >= timedelta(minutes=self.queue_aging_interval_minutes):
            ...
            if(self.scratch.chatting_patient):
                ...
                self.scratch.time_to_next = curr_time + timedelta(minutes=self.doctor_resting_time)
    ...
    def decide_to_chat(self, target_persona):
        ...
        if(target_persona.name in str(self.scratch.next_step)):
            if (
                ...
                event = target_persona.select_conversation_event(
                    [
                        target_persona.EVENT_TEST_RESULT_READY,
                        target_persona.EVENT_DISPOSITION_CHANGED,
                        target_persona.EVENT_TEST_ORDERED,
                        target_persona.EVENT_DOCTOR_FIRST_ASSESS,
                    ]
                )
                if event:
                    self.scratch.next_step = None
