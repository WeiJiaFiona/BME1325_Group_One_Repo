...
class TestEnsureCompressed:
    ...
    def test_refresh_removes_existing_compressed_snapshot(self, tmp_path):
        sim_code = "my-sim"
        compressed = tmp_path / "compressed_storage" / sim_code
        compressed.mkdir(parents=True)
        (compressed / "master_movement.json").write_text("{}")
        storage = tmp_path / "storage" / sim_code
        storage.mkdir(parents=True)

        def fake_run(*args, **kwargs):
            compressed.mkdir(parents=True, exist_ok=True)
            (compressed / "master_movement.json").write_text("{\"0\": {}}")
            return MagicMock(returncode=0, stdout="", stderr="")

        with patch("compute_metrics.FRONTEND_ROOT", tmp_path), \
             patch("compute_metrics.REVERIE_ROOT", tmp_path), \
             patch("compute_metrics.subprocess.run", side_effect=fake_run) as mock_run:
            ensure_compressed(sim_code, refresh=True)

        mock_run.assert_called_once()
        assert json.loads((compressed / "master_movement.json").read_text()) == {"0": {}}
