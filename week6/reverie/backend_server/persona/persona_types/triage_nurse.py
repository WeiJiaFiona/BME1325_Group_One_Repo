...
from triage_policy import resolve_patient_priority_and_zone

class Triage_Nurse(Persona):
    priority_factor = 0
    triage_standard = "CN_AD"
    ...
    def move(self, maze, personas, curr_tile, curr_time, data_collection):
        ...
        if(self.scratch.chatting_with == None):
            ...
            if(self.scratch.chatting_patient):
                ...
                if patient:
                    triage_info = resolve_patient_priority_and_zone(
                        patient,
                        triage_standard=self.triage_standard,
                    )
                    ctas = triage_info["ctas"]
                    if getattr(patient.scratch, "CTAS", None) is None:
                        patient.scratch.CTAS = ctas
                    if not getattr(patient.scratch, "injuries_zone", None):
                        patient.scratch.injuries_zone = triage_info["injuries_zone"]
                    bisect.insort_right(
