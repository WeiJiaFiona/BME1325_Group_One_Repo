...
OUTPUT_RESOURCE = REPO_ROOT / 'analysis' / 'resource_event_metrics.json'
...
def ensure_compressed(sim_code: str, *, refresh: bool = False) -> None:
    ...
    if refresh and compressed_root.exists():
...
def parse_args():
    ...
    parser.add_argument(
        '--refresh-compressed',
        action='store_true',
        help='Delete any existing compressed snapshot for this sim and recompress from current storage before analysis.',
    )
...
if __name__ == "__main__":
    ...
    ensure_compressed(sim_code, refresh=args.refresh_compressed)
    ...
    for step_str in sorted(raw_movement.keys(), key=lambda value: int(value)):
        snapshot = raw_movement[step_str]
        step = int(step_str)
    ...
    records = []
    for name in sorted(patient_steps.keys()):
        series = patient_steps[name]
        ...
        records.append({
            ...
            'testing_kind': info.get('testing_kind'),
            'boarding_timeout_occurred': bool(
                (info.get('boarding_timeout_event') or {}).get('occurred', False)
            ),
        })
    ...
    resource_summary = {
        'simulation': sim_code,
        'arrival_profile_mode': meta.get('arrival_profile_mode', 'normal'),
        'lab_capacity': meta.get('lab_capacity'),
        'lab_turnaround_minutes': meta.get('lab_turnaround_minutes'),
        'imaging_capacity': meta.get('imaging_capacity'),
        'imaging_turnaround_minutes': meta.get('imaging_turnaround_minutes'),
        'boarding_timeout_minutes': meta.get('boarding_timeout_minutes'),
        'lab_patients': int((patient_df['testing_kind'] == 'lab').sum()) if not patient_df.empty else 0,
        'imaging_patients': int((patient_df['testing_kind'] == 'imaging').sum()) if not patient_df.empty else 0,
        'boarding_timeout_events': int(patient_df['boarding_timeout_occurred'].fillna(False).sum()) if not patient_df.empty else 0,
    }
    OUTPUT_RESOURCE.write_text(json.dumps(resource_summary, indent=2), encoding='utf-8')
    ...
    print(f'Wrote resource/event metrics to {OUTPUT_RESOURCE}')
