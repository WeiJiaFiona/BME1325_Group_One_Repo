...
def _atomic_write_json(path, data, indent=2, retries=5, retry_delay=0.05):
  ...
  try:
    ...
    for attempt in range(retries + 1):
      try:
        ...
        last_error = exc
        try:
          # Windows can transiently deny os.replace when the frontend is
          # polling the destination file. Fall back to an in-place rewrite so
          # step output is still published instead of aborting the simulation
          # loop.
...
class ReverieServer: 
  def __init__(self, 
    ...
    utils.log_runtime_event(
      ...
      extra={"fork_sim_code": fork_sim_code, "temp_storage": fs_temp_storage},
    )
    ...
    utils.log_runtime_event(
      ...
      extra={"meta_path": self.meta_path},
    )
    ...
    Patient.imaging_turnaround_minutes = self.imaging_turnaround_minutes
    Patient.boarding_timeout_minutes = self.boarding_timeout_minutes
    ...
    _atomic_write_json(f"{fs_temp_storage}/curr_sim_code.json", curr_sim_code)
    
    curr_step = dict()
    curr_step["step"] = self.step
    _atomic_write_json(f"{fs_temp_storage}/curr_step.json", curr_step)
    utils.log_runtime_event(
      "frontend sync files initialised",
      sim_code=self.sim_code,
      step=self.step,
      extra={
        "curr_sim_code_path": f"{fs_temp_storage}/curr_sim_code.json",
  ...
  def _runtime_timer(self, label, *, step=None, command=None, extra=None):
    return utils.runtime_timed(
      label,
      sim_code=self.sim_code,
      step=self.step if step is None else step,
      command=command,
  ...
  def _write_sim_status(self, sim_folder):
    ...
    if self.step % self.status_interval_steps != 0:
      return
    ...
    lab_in_progress = len(getattr(self.maze, "lab_patients", []))
    imaging_in_progress = len(getattr(self.maze, "imaging_patients", []))
    lab_waiting = 0
    imaging_waiting = 0
    ...
    lines.append(f"    Lab waiting                {lab_waiting:>4}")
    lines.append(f"    Imaging waiting            {imaging_waiting:>4}")
    lines.append("")
    ...
    status_json = {
      ...
      "queues": {
        ...
        "imaging_waiting": imaging_waiting,
      },
      "resources": {
        "arrival_profile_mode": self.arrival_profile_mode,
    ...
    if not status_written:
      try:
        ...
        pass
      self._runtime_log(
        "sim status snapshot skipped",
        extra={
          "status_path": json_path,
          "reason": "target locked by another process",
        },
      )
      return
    self._runtime_log(
  ...
  def save(self): 
    ...
    reverie_meta["imaging_turnaround_minutes"] = self.imaging_turnaround_minutes
    reverie_meta["boarding_timeout_minutes"] = self.boarding_timeout_minutes
  ...
  def start_server(self, int_counter):
    ...
    """
    self._runtime_log(
      "start_server invoked",
      extra={"requested_steps": int_counter, "sim_time": self.curr_time.strftime("%B %d, %Y, %H:%M:%S")},
    )
    ...
    while (True):
      ...
      self._runtime_log(
        "curr_step published",
        step=self.step,
        extra={"curr_step_path": f"{fs_temp_storage}/curr_step.json"},
      )
  ...
  def open_server(self, frontend_ui: str = None, input_command: str = None) -> None:
    ...
    self._runtime_log(
      ...
      extra={
        ...
        "curr_step_note": "curr_step.json updates after a run command finishes",
      },
    )
    ...
    def _safe_unlink(path_obj, retries=5, delay_seconds=0.05):
        if not path_obj:
            return
    ...
    while True:
        ...
        try:
          ...
          command_wall_start = time.perf_counter()
          self._runtime_log("command received", command=sim_command)
          ...
          if cmd_lower in ["f", "fin", "finish", "save and finish"]:
              ...
              self._runtime_log(
                "command finished",
                command=sim_command,
                elapsed_seconds=time.perf_counter() - command_wall_start,
              )
              finished = True
              ...
              ret_str = (
                ...
                f"curr_step.json updates after each run command completes.\n"
                f"Wall-clock duration: {elapsed:.3f}s\n"
              )
              self._runtime_log(
                "command finished",
                command=sim_command,
            ...
            self._runtime_log(
              "command failed",
              command=sim_command,
              elapsed_seconds=time.perf_counter() - command_wall_start if 'command_wall_start' in locals() else None,
              extra={"error": str(e)},
            )
