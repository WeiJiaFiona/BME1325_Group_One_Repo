...
def user_mode_chat_turn(message: str) -> Dict[str, Any]:
    ...
    if session["phase"] == "CALL_NURSE_MEASURE":
        ...
        if retrieval_result.get("fallback_used", False):
            ready_for_disposition = old_ready
        else:
            ready_for_disposition = old_ready and _doctor_ready_stage2(
                session,
                retrieval_result=retrieval_result,
                plan_contract=plan_contract,
            )
        ...
        if not ready_for_disposition:
            if _is_imaging_question(msg):
                ...
                imaging_line = _agent_reply(
                    ...
                    session,
                    extra={
                        "mode": "patient_question_answer",
                        "primary_protocol_id": primary,
                        "patient_message": msg,
                        "instruction": (
                            "Patient asked whether imaging is needed. "
                            "Give one concise recommendation with likely modality and safety caveat."
                        ),
                    },
                    use_llm=True,
                )
