...
def home(request):
    ...
    # Keep the template context available to Django tests even when the
    # response object is consumed outside the standard template signal path.
    setattr(response, "context", context)
    setattr(response, "context_data", context)
    return response
