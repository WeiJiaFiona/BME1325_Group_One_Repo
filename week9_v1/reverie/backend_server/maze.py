...
class Maze: 
  def __init__(self, maze_name, fork_folder, sim_folder,seed): 
    ...
    self.lab_patients = meta_info.get("lab_patients", [])
    self.imaging_patients = meta_info.get("imaging_patients", [])
    self.lab_capacity = meta_info.get("lab_capacity", 0)
    self.imaging_capacity = meta_info.get(
      "imaging_capacity",
      self.injuries_zones.get("diagnostic room", {}).get("capacity", 0),
    )
    # Initialize beds
  ...
  def save(self, reverie_maze_f):
    ...
    maze_meta["lab_patients"] = self.lab_patients
    maze_meta["imaging_patients"] = self.imaging_patients
    maze_meta["lab_capacity"] = self.lab_capacity
    maze_meta["imaging_capacity"] = self.imaging_capacity
    ...
    EXAMPLE OUTPUT
      ...
      self.tiles[9][58] = {'world': 'double studio', 
            ...
            'events': {('double studio:double studio:bedroom 2:bed',
                       None, None)}} 
    """
    if not self.is_tile_in_bounds(tile):
      raise IndexError(f"Tile out of maze bounds: {tile}")
    x = tile[0]
    y = tile[1]
    return self.tiles[y][x]


  def is_tile_in_bounds(self, tile):
    try:
      x = int(tile[0])
      y = int(tile[1])
  ...
  def _tile_events_or_none(self, tile, action):
    if not self.is_tile_in_bounds(tile):
      print(f"(maze): Warning - skip {action}; tile out of bounds: {tile}")
      return None
    return self.tiles[int(tile[1])][int(tile[0])]["events"]
  ...
  def add_event_from_tile(self, curr_event, tile): 
    ...
    OUPUT: 
      None
    """
    events = self._tile_events_or_none(tile, "add_event_from_tile")
    if events is None:
      return False
  ...
  def remove_event_from_tile(self, curr_event, tile):
    ...
    OUPUT: 
      None
    """
    events = self._tile_events_or_none(tile, "remove_event_from_tile")
    if events is None:
      return False
    curr_tile_ev_cp = events.copy()
  ...
  def turn_event_from_tile_idle(self, curr_event, tile):
    events = self._tile_events_or_none(tile, "turn_event_from_tile_idle")
    if events is None:
      return False
    curr_tile_ev_cp = events.copy()
    for event in curr_tile_ev_cp: 
  ...
  def remove_subject_events_from_tile(self, subject, tile):
    ...
    OUPUT: 
      None
    """
    events = self._tile_events_or_none(tile, "remove_subject_events_from_tile")
    if events is None:
      return False
    curr_tile_ev_cp = events.copy()
    for event in curr_tile_ev_cp: 
