...
def generate_wake_up_hour(persona):
  ...
  Persona state: identity stable set, work_pattern, first_name
...
def generate_first_daily_plan(persona, wake_up_hour): 
  ...
  Persona state: identity stable set, work_pattern, cur_data_str, first_name
  ...
  result = run_gpt_prompt_daily_plan(persona, wake_up_hour)[0]
  if len(result) == 0:
     print("ERROR: generate_first_daily_plan failed")
  return result
...
def generate_action_game_object(act_desp, act_address, persona, maze):
  ...
  EXAMPLE OUTPUT: 
    "bed"
  """
  act_address = act_address.replace("traige", "triage")
...
def generate_act_obj_desc(act_game_object, act_desp, persona): 
  ...
  print ("DEBUG::: ", act_game_object, act_desp)
...
def generate_convo_summary(persona, convo):
  ...
  if not result:
    ...
    return all_utt.strip() if all_utt else "A conversation took place."
  return result[0]
...
def revise_identity(persona): 
  ...
  currently_prompt = f"{p_name}'s status from {(persona.scratch.curr_time - timedelta(days=1)).strftime('%A %B %d')}:\n"
  currently_prompt += f"{persona.scratch.current_state}\n\n"
  currently_prompt += f"{p_name}'s thoughts at the end of {(persona.scratch.curr_time - timedelta(days=1)).strftime('%A %B %d')}:\n" 
  currently_prompt += (plan_note + thought_note).replace('\n', '') + "\n\n"
  currently_prompt += f"It is now {persona.scratch.curr_time.strftime('%A %B %d')}. Given the above, write {p_name}'s status for {persona.scratch.curr_time.strftime('%A %B %d')} that reflects {p_name}'s thoughts at the end of {(persona.scratch.curr_time - timedelta(days=1)).strftime('%A %B %d')}. Write this in third-person talking about {p_name}."
  ...
  persona.scratch.current_state = new_currently
  ...
  persona.scratch.schedule_constraints = new_daily_req
...
def _long_term_planning(persona, new_day): 
  ...
  print("______________________________new day______________________________", new_day)
  # When it is a new day, we start by creating the daily_req of the persona.
  # Note that the daily_req is a list of strings that describe the persona's
  # day in broad strokes.
  if new_day == "First day": 
    print("IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIINow is the new dayIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII")
    ...
    print("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxNow is the new dayxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
...
def _determine_action(persona, maze): 
  ...
  print(curr_index, "f_daily_schedule", len(persona.scratch.f_daily_schedule))
  if curr_index == 0:
...
def _choose_retrieved(persona, retrieved): 
  ...
  for event_desc, rel_ctx in retrieved.items(): 
    ...
    if (":" not in curr_event.subject 
        and curr_event.subject != persona.name or curr_event.subject in str(persona.scratch.next_step)): 
      priority += [rel_ctx]
  if priority: 
...
def _should_react(persona, retrieved, personas): 
  ...
  def lets_talk(init_persona, target_persona, retrieved):
    if (target_persona.scratch.chatting_with 
      ...
      return False
    # Different roles have different rules on who to chat to
    result = init_persona.decide_to_chat(target_persona)  
  ...
  def lets_react(init_persona, target_persona, retrieved): 
    ...
    if react_mode == "1": 
      wait_until = ((target_persona.scratch.act_start_time 
        + timedelta(minutes=target_persona.scratch.act_duration - 1))
  ...
  if ":" not in curr_event.subject: 
    ...
    if(curr_event.subject in personas.keys()):
...
def _create_react(persona, inserted_act, inserted_act_dur,
  ...
  if(p.role == None):
    ...
    elif (p.scratch.f_daily_schedule_hourly_org[p.scratch.get_f_daily_schedule_hourly_org_index()][1] and 
        len(p.scratch.f_daily_schedule_hourly_org) > p.scratch.get_f_daily_schedule_hourly_org_index()+1
        ): 
...
def _chat_react(maze, persona, focused_event, reaction_mode, personas):
  ...
  target_persona = personas.get(target_name)
  ...
  init_persona.react_to_chat(convo_summary, target_persona, maze)
  target_persona.react_to_chat(convo_summary, init_persona, maze)
  ...
  if curr_time.second != 0: 
    temp_curr_time = curr_time + timedelta(seconds=60 - curr_time.second)
    chatting_end_time = temp_curr_time + timedelta(minutes=inserted_act_dur)
  else: 
    chatting_end_time = curr_time + timedelta(minutes=inserted_act_dur)
...
def plan(persona, maze, personas, new_day, retrieved): 
  ...
  # print("00000000000000000000000000000new day", new_day)
  ...
  if(persona.scratch.chatting_end_time):
      if (persona.scratch.chatting_end_time <= persona.scratch.curr_time):
